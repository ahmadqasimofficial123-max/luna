...
export default function AIAgent() {
  ...
  const chat = trpc.ai.chat.useMutation({ onSuccess: ({ content }) => setMessages(current => [...current, { role: "assistant", content }]), onError: error => setMessages(current => [...current, { role: "assistant", content: `I couldn’t answer that right now. ${error.message}` }]) });
