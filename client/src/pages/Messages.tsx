...
export default function Messages() {
  ...
  return <div className={`messages-page ${selectedId ? "has-selection" : "no-selection"}`}>
    <aside className="messages-sidebar">
      <div className="messages-sidebar-head"><div><p className="eyebrow">YOUR PRIVATE ORBIT</p><h1>Messages</h1></div><Button className="primary-button" size="icon" aria-label="Start a new message" onClick={() => toast.info("Choose a conversation to start a new message")}><FilePlus2 size={17} /></Button></div>
      <div className="message-search"><Search size={16} /><Input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search conversations" /></div>
      <div className="message-filters">{(["all", "unread", "groups"] as const).map(item => <button type="button" key={item} className={filter === item ? "active" : ""} onClick={() => setFilter(item)}>{item[0].toUpperCase() + item.slice(1)}{item === "unread" && <span>3</span>}</button>)}</div>
