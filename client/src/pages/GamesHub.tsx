...
export default function GamesHub() {
  ...
  const categories = useMemo(() => ["All", ...Array.from(new Set(HUB_GAMES.map(game => game.category || "Featured")))], []);
  const visibleGames = useMemo(() => category === "All" ? HUB_GAMES : HUB_GAMES.filter(game => (game.category || "Featured") === category), [category]);
  ...
  return <GameSidebar><main className="games-hub-page">
    ...
    <div className="games-hub-content">
      <section className="games-hub-hero"><div><p className="eyebrow">THE LUNA ARCADE</p><h1>Play something<br /><em>unexpected.</em></h1><p>Pick a game and jump in. Every title opens inside Luna Social, so your game session stays in the same orbit.</p></div><div className="games-hub-stat"><strong>{HUB_GAMES.length}</strong><span>unique games ready<br />for your orbit</span></div></section>
      <div className="games-filter-row" aria-label="Filter games">{categories.map(item => <button type="button" key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div>
      <section className="games-grid" aria-label="Available games">{visibleGames.map(game => <article className="game-card" key={game.slug} style={{ "--card-accent": game.accent } as React.CSSProperties}>
