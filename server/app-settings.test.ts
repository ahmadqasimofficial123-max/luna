...
const dbMocks = vi.hoisted(() => ({
  ...
  updateAppSettings: vi.fn().mockResolvedValue({ id: 1, appName: "Nightline", tagline: "A calmer place to connect.", theme: "light", accentColor: "#ff7ac8", logoUrl: "https://cdn.example.com/nightline.png", updatedBy: 8 }),
}));
...
describe("app settings procedures", () => {
  it("allows public reads and forwards a valid admin update", async () => {
    ...
    await expect(adminCaller.social.updateAppSettings({ appName: "Nightline", tagline: "A calmer place to connect.", theme: "light", accentColor: "#ff7ac8", logoUrl: "https://cdn.example.com/nightline.png" })).resolves.toMatchObject({ appName: "Nightline", logoUrl: "https://cdn.example.com/nightline.png" });
    expect(dbMocks.updateAppSettings).toHaveBeenCalledWith(8, { appName: "Nightline", tagline: "A calmer place to connect.", theme: "light", accentColor: "#ff7ac8", logoUrl: "https://cdn.example.com/nightline.png" });
  });
  ...
  it("blocks non-admin writes and rejects unsafe or empty patches", async () => {
    ...
    await expect(adminCaller.social.updateAppSettings({ logoUrl: "javascript:alert(1)" as never })).rejects.toThrow();
