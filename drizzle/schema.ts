...
export const postHides = mysqlTable("postHides", { id: int("id").autoincrement().primaryKey(), userId: int("userId").notNull(), postId: int("postId").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull() }, (table) => ({ uniqueHide: uniqueIndex("unique_post_hide").on(table.userId, table.postId) }));
export const postShares = mysqlTable("postShares", { id: int("id").autoincrement().primaryKey(), userId: int("userId").notNull(), postId: int("postId").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull() });
...
export const messageReactions = mysqlTable("messageReactions", { id: int("id").autoincrement().primaryKey(), messageId: int("messageId").notNull(), userId: int("userId").notNull(), reaction: varchar("reaction", { length: 16 }).notNull(), createdAt: timestamp("createdAt").defaultNow().notNull() }, (table) => ({ uniqueMessageReaction: uniqueIndex("unique_message_reaction").on(table.messageId, table.userId) }));
export const messageReads = mysqlTable("messageReads", { id: int("id").autoincrement().primaryKey(), messageId: int("messageId").notNull(), userId: int("userId").notNull(), readAt: timestamp("readAt").defaultNow().notNull() }, (table) => ({ uniqueMessageRead: uniqueIndex("unique_message_read").on(table.messageId, table.userId) }));

export const notifications = mysqlTable("notifications", { id: int("id").autoincrement().primaryKey(), recipientId: int("recipientId").notNull(), actorId: int("actorId"), type: mysqlEnum("type", ["like", "comment", "follow", "follow_request", "mention", "story_reaction", "message", "profile_update"]).notNull(), entityId: int("entityId"), readAt: timestamp("readAt"), createdAt: timestamp("createdAt").defaultNow().notNull() });
export const notificationHides = mysqlTable("notificationHides", { id: int("id").autoincrement().primaryKey(), userId: int("userId").notNull(), notificationId: int("notificationId").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull() }, (table) => ({ uniqueHide: uniqueIndex("unique_notification_hide").on(table.userId, table.notificationId) }));
export const notificationMutes = mysqlTable("notificationMutes", { id: int("id").autoincrement().primaryKey(), userId: int("userId").notNull(), notificationType: varchar("notificationType", { length: 32 }).notNull(), createdAt: timestamp("createdAt").defaultNow().notNull() }, (table) => ({ uniqueMute: uniqueIndex("unique_notification_mute").on(table.userId, table.notificationType) }));
export const notificationActions = mysqlTable("notificationActions", { id: int("id").autoincrement().primaryKey(), userId: int("userId").notNull(), notificationId: int("notificationId").notNull(), action: varchar("action", { length: 32 }).notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull() }, (table) => ({ uniqueAction: uniqueIndex("unique_notification_action").on(table.userId, table.notificationId) }));
...
export const appSettings = mysqlTable("appSettings", { id: int("id").autoincrement().primaryKey(), appName: varchar("appName", { length: 80 }).default("Luna Social").notNull(), tagline: varchar("tagline", { length: 180 }).default("Find your people under the same sky.").notNull(), theme: mysqlEnum("theme", ["dark", "light", "system"]).default("dark").notNull(), accentColor: varchar("accentColor", { length: 7 }).default("#a98cff").notNull(), logoUrl: text("logoUrl"), updatedBy: int("updatedBy") });
